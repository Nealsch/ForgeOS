...
Cursor is **not currently in active use** on ForgeOS. This profile exists so the platform matrix remains discoverable and the folder structure does not need to change when Cursor is later adopted.
...
| Runtime config | TBD on adoption (likely `.cursorrules` and/or `.cursor/`) |
...
This stub records the platform's identity, the architectural role it would fulfil, and the fact that it is not yet in use. It does not speculatively document Cursor's features or runtime configuration.

---

## Expansion Checklist (when adopted)

When Cursor is adopted on a ForgeOS project, expand this profile in the same pull request that introduces its runtime configuration. Minimum content:
...
Until expanded, this profile defines nothing about how Cursor operates. All engineering authority remains with the Framework.
