...
Claude Code is **not currently in active use** on ForgeOS. This profile exists so the platform matrix remains discoverable and the folder structure does not need to change when Claude Code is later adopted.
...
| Type | Terminal/CLI AI coding assistant (Anthropic) |
...
* The architectural role it would fulfil.
* The fact that it is not yet in use.

It does **not** speculatively document Claude Code's features, capabilities, or runtime configuration.

---

## Expansion Checklist (when adopted)

When Claude Code is adopted on a ForgeOS project, expand this profile in the same pull request that introduces its runtime configuration. Minimum content:
...
Until expanded, this profile defines nothing about how Claude Code operates. All engineering authority remains with the Framework.
